...
3. 設定：
   ...
   - **Start Command**：`gunicorn -b 0.0.0.0:$PORT app:app --workers 1 --timeout 180`（單 worker 省記憶體；timeout 180 秒避免較慢 API 被殺）
   - **502 常見原因**：Render 有約 30 秒請求上限，若 API 處理過久會直接回 502。**設定 FMP_API_KEY** 讓美股指數一次請求取得（最快）；或 **FINNHUB_API_KEY** 作備援。冷啟動後第一次請求較慢，可多按一次「更新」重試。
...
**選填環境變數（多資料源）**：  
- **FMP_API_KEY**：美股指數一次請求取得（最快）、美股財報行事曆（[申請](https://site.financialmodelingprep.com/register)）。**建議優先設定**，可大幅減少 502。  
- **FINNHUB_API_KEY**：美股指數與美股個股備援（免費 60 次/分，[申請](https://finnhub.io/)）。未設 FMP 時使用。  
- **TWELVEDATA_API_KEY**：重要比率歷史圖備援。雲端（如 Render）上 yfinance 常被擋，導致「無歷史資料可顯示」；設定此 key 後會改用 Twelve Data（[申請](https://twelvedata.com/register)，免費 8 次/分）。  
- 台股財報維持 yfinance（含 calendar fallback）；加密貨幣一律用 Deribit（無需 key）。  
