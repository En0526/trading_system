...
3. 設定：
   ...
   - **502 常見原因**：Render 有約 30 秒請求上限，若 API 處理過久會直接回 502。**設定 FINNHUB_API_KEY** 可讓美股指數改由 Finnhub 取得（通常比 yfinance 快），減少超時。冷啟動後第一次請求較慢，可多按一次「更新」重試。
...
- **FMP_API_KEY**：美股財報行事曆（[申請](https://site.financialmodelingprep.com/register)）。未設則用 yfinance。  
