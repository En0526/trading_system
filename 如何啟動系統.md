...
- **FMP_API_KEY**：財報行事曆備援（[申請](https://site.financialmodelingprep.com/register)）。未設則用 yfinance。  
- **TWELVEDATA_API_KEY**：重要比率歷史圖備援。雲端（如 Render）上 yfinance 常被擋，導致「無歷史資料可顯示」；設定此 key 後會改用 Twelve Data 取得金銀比、比特黃金比等歷史序列（[申請](https://twelvedata.com/register)，免費 8 次/分）。  
- 台股維持 yfinance；加密貨幣一律用 Deribit（無需 key）。  
